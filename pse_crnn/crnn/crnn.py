...
from .crnn_utils import StrLabelConverter, ResizeNormalize


converter = StrLabelConverter(''.join(alphabet))


class CRNNHandle(object):
    def __init__(self, model_path, net, gpu_id=None):
        """
        Args:
            model_path: 模型地址(可以是模型的参数或者参数和计算图一起保存的文件)
            net: 网络计算图，如果在model_path中指定的是参数的保存路径，则需要给出网络的计算图
            gpu_id: 在哪一块gpu上运行
        """
